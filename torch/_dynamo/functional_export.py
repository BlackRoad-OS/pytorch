...
def make_fx_graph_from_captured_output(
    out: CaptureOutput, mod: Any, args: tuple[Any, ...], kwargs: dict[str, Any]
) -> torch.fx.GraphModule:
    assert out.backend_input is not None
    backend_input = out.backend_input
    ...
    class DynamoBytecodeUnflatten:
        ...
        def __call__(self, flat_outs, inputs):
            def backend_dummy(*example_inputs):
                return flat_outs
    ...
    if isinstance(root, torch.nn.Module):
        ...
        if graph_module._forward_hooks or graph_module._forward_pre_hooks:
            ...
            def dynamo_wrapped_call(self, *args, **kwargs):
    ...
    dynamo_bytecode_flatten = DynamoBytecodeFlatten(input_processor)
    dynamo_bytecode_unflatten = DynamoBytecodeUnflatten(input_processor)
...
class _DynamoBytecodeCodeGen(torch.fx.graph.CodeGen):
    ...
    def process_outputs(self, out: Any) -> Any:
        results = self.dynamo_bytecode_unflatten(out, self._inputs)
        if self.wrap_tuple:
            results = (results,)
        self._inputs = None
        return results
...
def dynamo_graph_capture_for_export(
    ...
    def inner(*args: Any, **kwargs: Any) -> Any:
        ...
        return make_fx_graph_from_captured_output(out, mod, args, kwargs)
